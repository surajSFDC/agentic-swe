...
4. Create `.claude/.work/<id>/state.json` from `templates/state.json`, filling in:
...
6. Branch setup (delegate to `agents/git-ops.md` if complex):
...
- Execute phases using the matching prompt in `phases/`
- Follow either:
...
- When resuming at `approval-wait`:
  - If `approvals.pr_approved == true`: execute `phases/completion.md`, transition to `completed`.
