...
Follow the same delegation model as `phases/implementation.md`, with additional subagent auto-selection for fast path:
...
5. Spawn `agents/developer.md` (primary, **foreground**) with the task scope, target files, and constraints. Tell the developer agent it may spawn 1 subagent per `phases/subagent-selection.md` if it encounters domain-specific complexity.
