...
2. Invoke `/diff-review` against the changed files for structured, evidence-backed findings.
3. Compare the code against required behavior, design intent, repo conventions, and likely edge cases.
4. Review with a defect-oriented mindset:
   - wrong or partial behavior
   - missing failure case handling
   - unjustified complexity
   - poor test coverage for risky branches
5. Separate blocking findings, non-blocking concerns, and speculative risks.
6. Score the implementation against each dimension (1=fail, 2=acceptable, 3=strong):
...
7. Verdict rule: any dimension scored 1 → rejected. All 2+ → approved.
...
Following `templates/artifact-format.md`, include:
