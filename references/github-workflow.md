...
For step-by-step branch, sync, and conflict procedures, see `${CLAUDE_PLUGIN_ROOT}/agents/git-operations-agent.md`. This reference provides the authoritative facts those procedures rely on.
...
Inspect markers, preserve upstream fixes, abort when unsafe. See `${CLAUDE_PLUGIN_ROOT}/agents/git-operations-agent.md` Conflict Rules for procedure.

## Fork and Upstream Sync

Verify remotes: `origin` = fork, `upstream` = original. Add `upstream` if missing. See `${CLAUDE_PLUGIN_ROOT}/agents/git-operations-agent.md` for sync procedure.
...
- record the actual PR URL in `.worklogs/<id>/pr-link.txt`
- record important git workflow actions in `.worklogs/<id>/progress.md`
- record blockers and failed workflow attempts in `.worklogs/<id>/audit.log`
