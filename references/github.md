...
For step-by-step branch, sync, and conflict procedures, see `agents/git-ops.md`. This reference provides the authoritative facts those procedures rely on.
...
Inspect markers, preserve upstream fixes, abort when unsafe. See `agents/git-ops.md` Conflict Rules for procedure.

## Fork and Upstream Sync

Verify remotes: `origin` = fork, `upstream` = original. Add `upstream` if missing. See `agents/git-ops.md` for sync procedure.
